...
- 选取一个轴pivot，一般默认选择第一个元素作为pivot，从数组右边开始遍历，如果nums[right]<pivot,把nums[right]赋值给pivot空出来的位置，然后从左边开始遍历，如果nums[left]>pivot,把nums[left]赋值给刚刚nums[right]的位置，不断左右接力‘挖坑’、‘填坑’，完成一次快排
- 递归左右两侧的数组
...
```
  /**
    ...
    private void sort(int[] nums, int left, int right) {
        //递归函数的临界点left<right，没有临界点递归函数会死循环
        if (left < right) {
            //一次快排，pivot左边都比pivot小，右边都比pivot大
            int pivotIndex = patition(nums, left, right);
            //递归左边
...
- 选取2个轴，一般默认数组第一个元素为轴p1，最后一个元素为轴p2，lt和gt分别由低位和高位进行遍历，将整个数组分成3部分。第一部分：<pivot1的元素;第二部分：>=pivot1 && <pivot2的元素;第三部分：>pivot2的元素
- 递归这三部分

![](https://ws3.sinaimg.cn/large/006tKfTcgy1frdkdurgqoj318i0ue13e.jpg)
**双轴快排代码：**
...
```
    ...
    private void sort(int[] nums, int left, int right) {
        ...
        if (left < right) {
            //一次快排，pivot左边都比pivot小，右边都比pivot大
            int[] pivotIndexs = patition(nums, left, right);
            int pivot1Index = pivotIndexs[0];
            int pivot2Index = pivotIndexs[1];
            //递归<pivot1的
            sort(nums, left, pivot1Index - 1);
            //递归>=pivot1 && <=pivot2的
            sort(nums, pivot1Index + 1, pivot2Index - 1);
            //递归>pivot2的
            sort(nums, pivot2Index + 1, right);
        }
    }
